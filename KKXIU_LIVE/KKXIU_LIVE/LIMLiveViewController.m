//
//  LIMLiveViewController.m
//  KKXIU_LIVE
//
//  Created by 张洋 on 2017/8/22.
...
//#define webUrl @"http://app.game.kkxiu.com/scjl/"
//#define webUrl [NSString stringWithFormat:@"http://127.0.0.1:%d/zww/zww.html",mLocalPort]


@interface LIMLiveViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UITableViewDelegate,UITableViewDataSource,LiveGiftShowCustomDelegate,WKNavigationDelegate,WKScriptMessageHandler,WKUIDelegate>
/////////////////////
...
@property (nonatomic ,assign) NSString *currentGiftCount;


@property (nonatomic, strong) UIView *liveEndView;
...
@property (nonatomic, assign) NSInteger currentAnimation;
...
@implementation LIMLiveViewController
...
- (void)setBackUI{
    ...
    if (self.coverUrl.length >0) {
        _playView.image = [UIImage imageNamed:self.coverUrl];
    }else{
        _playView.image = [UIImage imageNamed:@"login_back"];
    }
    ...
    [self.txLivePlayer setupVideoWidget:[UIScreen mainScreen].bounds containView:_playView insertIndex:0];
...
//    _fontView.backgroundColor = [UIColor colorWithWhite:1 alpha:0.1];
//    
//    // 关闭按钮
...
//    upImageV.layer.cornerRadius = upH/2;
...
//    userView.backgroundColor = [UIColor colorWithWhite:1 alpha:0.5];
//    
//    [_fontView addSubview:userView];
...
//    
//    [_fontView addSubview:presentView];
...
//        make.width.mas_equalTo(kScreenW);
//        make.height.mas_equalTo(195 *kiphone6);
//    }];
//    UIView *toolView = [[UIView alloc]init];
//    toolView.backgroundColor = [UIColor colorWithWhite:0.8 alpha:0.5];
//    
//    [_fontView addSubview:toolView];
//    [toolView mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.bottom.equalTo(presentView.mas_top).offset(0);
//        make.left.equalTo(ws.fontView).offset(0);
//        make.width.mas_equalTo(kScreenW);
...
- (void)httpForPushInfo{
    ...
    NSLog(@"= 请求 数据 =    %@",userModel);
    
    [paraDict addEntriesFromDictionary:[userModel httpParaDictSecret:@{@"liveuid":self.liveuid,@"roomid":self.roomid}]];
    [paraDict addEntriesFromDictionary:[userModel httpParaDictUnSecret]];
    
    
    
//    NSLog(@"secret = %@,  unsecret = %@,",[userModel httpParaDictSecret],[userModel httpParaDictUnSecret]);
    
    NSLog(@"%@   dict = ",paraDict);
    [[HttpClient defaultClient] requestWithPath:mLive_Login method:1 parameters:paraDict prepareExecute:^{
        ...
        if ([rescode isEqualToString:@"1"]) {
            ...
            NSDictionary * liveDict = responseObject[@"result"][@"compereinfo"];
            LIMPushLiveModel *livePushModel = [LIMPushLiveModel mj_objectWithKeyValues:liveDict];
            
            self.livePushModel = livePushModel;
            NSDictionary * roominfoDict = responseObject[@"result"][@"roominfo"];
            NSLog(@"roominfo = %@",roominfoDict);

             //使用 startPlay 即可开始播放，如果是直播的话我们推荐 FLV 协议
            [_txLivePlayer startPlay:roominfoDict[@"playvideourl"] type:PLAY_TYPE_LIVE_RTMP];
            self.groupID = roominfoDict[@"roomid"];
            self.gameID = [roominfoDict[@"gameid"] integerValue];

            
            
            [[TIMGroupManager sharedInstance] joinGroup:self.groupID msg:@"我是用户" succ:^{
                [self enterChat];
...
//            [self showAlertWithMessage:resmsg];
            ...
            self.liveEndView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kScreenW, kScreenH)];
            _liveEndView.hidden = YES;
            [self.view bringSubviewToFront:_liveEndView];
            _liveEndView.backgroundColor = [UIColor colorWithHexString:@"f2f2f2"];
            [self.view addSubview:_liveEndView];
            UIImageView *endImageV = [[UIImageView alloc]init];
            endImageV.image = [UIImage imageNamed:@"liveEnd2"];
            [self.liveEndView addSubview:endImageV];
            [endImageV mas_makeConstraints:^(MASConstraintMaker *make) {
                make.centerX.equalTo(_liveEndView).offset(0);
                make.top.equalTo(_liveEndView).offset(85.5);
                make.size.mas_equalTo(CGSizeMake( 250, 98));
            }];
            
            
            UIButton *backBtn = [UIButton buttonWithType:UIButtonTypeCustom];
            backBtn.backgroundColor = [UIColor colorWithHexString:@"ffcb00"];
            [backBtn setTitleColor:[UIColor colorWithHexString:@"fffefe"] forState:UIControlStateNormal];
            ...
            [_liveEndView addSubview:backBtn];
            [backBtn mas_makeConstraints:^(MASConstraintMaker *make) {
                make.centerX.equalTo(_liveEndView).offset(0);
                make.bottom.equalTo(_liveEndView).offset(-60.5);
                make.size.mas_equalTo(CGSizeMake( kScreenW -60, 44));
            }];

            [self.myTimer setFireDate:[NSDate distantFuture]];
            [self.view bringSubviewToFront:self.liveEndView];
...
//    [_customGiftShow setShowMode:fromTopToBottom];
...
//    [_customGiftShow setHiddenModel:left];
    ...
    // 礼物
//    self.turnView = [[LIMTurnHitView alloc]init];
...
//    }];
    ...
    //        make.width.mas_equalTo(kScreenW);
    //        make.height.mas_equalTo(160 *kiphone6 +35*kiphone6);
    //    }];
    ...
    [sendBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(moneyView).offset(2);
...
//    }];
    ...
    NSArray *toolIcon = @[@"pushpage_2",@"pushpage_3",@"pushpage_4"];
    for (int i = 0; i <3; i++) {
        UIButton *toolBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [toolBtn setImage:[UIImage imageNamed:toolIcon[i]] forState:UIControlStateNormal];
        [toolBtn sizeToFit];
        ...
        [toolBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.equalTo(toolView).offset(7 +i*41);
        }];
        if (i == 0) {
    ...
    [meauBtn addTarget:self action:@selector(meauClick) forControlEvents:UIControlEventTouchUpInside];
    [meauBtn sizeToFit];
...
//        make.bottom.equalTo(toolView.mas_top).offset(-20).priority(750);
...
//    }];
    ...
    [self.view addSubview:_fontView];
    [self.view bringSubviewToFront:_fontView];
    [self.view bringSubviewToFront:self.turnView];
...
//    UIWebView *gameWeb = [[UIWebView alloc]initWithFrame:CGRectMake(0, 0, kScreenW, kScreenH)];
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@?uid=%@&token=%@&deviceno=%@&roomid=%@&httphead=%@",gameUrl,userModel.uid,userModel.token,@"",self.groupID,httpHead]];
    NSLog(@"%@",[NSString stringWithFormat:@"%@?uid=%@&token=%@&deviceno=%@&roomid=%@&httphead=%@",gameUrl,userModel.uid,userModel.token,@"",self.groupID,httpHead]);
...
//    CcUserModel *userModel = [CcUserModel defaultClient];
    ...
    [gameWeb loadRequest:[NSURLRequest requestWithURL:url]];
...
- (void)meauClick{
//        NSString *inputValueJS = @"resetGame();";
//        //执行JS
...
//            self.tableView.frame = CGRectMake(2, kScreenH - 160*kiphone6-35*kiphone6 -40 -130, kScreenW *0.66, 120);
    ...
    if(recognizer.direction == UISwipeGestureRecognizerDirectionLeft) {
        NSLog(@"swipe left");
        [UIView animateWithDuration:0.5 animations:^{
            self.fontView.frame = CGRectMake(0, 0,kScreenW, kScreenH);
            self.turnView.frame = CGRectMake(0, 0,kScreenW, kScreenH);
        }];
...
{
    if (swit.isOn) {
        self.mainText.placeholder = @"开启弹幕，10K币/条";
    }else{
        self.mainText.placeholder = @"说点什么吧";
    }
...
//    }
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        ...
        [paraDict addEntriesFromDictionary:[userModel httpParaDictSecret:@{@"liveuid":self.liveuid,@"roomid":self.roomid}]];
        [paraDict addEntriesFromDictionary:[userModel httpParaDictUnSecret]];
        [[HttpClient defaultClient] requestWithPath:mLeaveLive method:1 parameters:paraDict prepareExecute:^{
...
//        NSLog(@"%@",responseObject);
        ...
        if ([rescode isEqualToString:@"1"]&&[userCount intValue] >0) {
...
//        CcUserModel *userModel = [CcUserModel defaultClient];
//        
//        NSMutableDictionary *paraDict = [[NSMutableDictionary alloc]initWithDictionary:@{@"liveuid":self.livePushModel.uid,@"giftid":model.giftid,@"giftcount":@"1"}];
//        [paraDict addEntriesFromDictionary:[userModel httpParaDictSecret]];
//        [paraDict addEntriesFromDictionary:[userModel httpParaDictUnSecret]];
...
{
    CGFloat systemKeyBoardHeight =[notification.userInfo[@"UIKeyboardBoundsUserInfoKey"]CGRectValue].size.height -500;
    ...
    [UIView animateWithDuration:0.5 animations:^{
        CGRect frame = self.tableView.frame;
        frame.origin.y = kScreenH-systemKeyBoardHeight-130-44;
        self.tableView.frame = frame;
...
- (void)receiveMessage:(NSNotification*)notification //键盘出现
{
//    CcUserModel *userModel = [CcUserModel defaultClient];
...
- (void)chatchatchat{
    ...
    CcUserModel *userModle = [CcUserModel defaultClient];
    //    self.mainText;
    NSDictionary *dict = @{
                           @"headPic":userModle.cover,
                           @"msg":self.mainText.text,
                           @"nickName":userModle.nickname,
                           @"userAction":number,
                           @"userId":userModle.uid,
                           @"level":userModle.userlevel,
                           @"isHost":userModle.iscompere
                           };
    ...
    [grp_conversation sendMessage:msg succ:^(){
        NSLog(@"SendMsg Succ");
        NSString *string = [NSString stringWithFormat:@"%@:%@",userModle.nickname,self.mainText.text];
        LIMChatModel *chatModel = [[LIMChatModel alloc]init];
        chatModel.nameLength = userModle.nickname.length +1;
        chatModel.sentence = string;
        chatModel.type = @"1";
        chatModel.isUp =  userModle.iscompere;
        chatModel.level = userModle.userlevel;
        chatModel.userName = userModle.nickname;
        chatModel.userIcon = userModle.cover;
...
//        NSDictionary *dict = @{@"headPic":self.livePushModel.avatar,
//                               @"msg":winMoney,
...
//                               @"isHost":userModel.iscompere};
        ...
        [grp_conversation sendMessage:msg succ:^(){
            NSLog(@"SendMsg Succ");
            NSString *string = [NSString stringWithFormat:@"%@ %@",userModle.nickname,text];
            LIMChatModel *chatModel = [[LIMChatModel alloc]init];
            chatModel.nameLength = userModle.nickname.length +1;
            chatModel.sentence = string;
            chatModel.type = @"9";
...
- (void)followOther:(NSString *)touid{
    ...
    NSMutableDictionary *paraDict = [[NSMutableDictionary alloc]initWithDictionary:@{}];
    [paraDict addEntriesFromDictionary:[userModel httpParaDictSecret:@{@"touid":self.livePushModel.uid}]];
    [paraDict addEntriesFromDictionary:[userModel httpParaDictUnSecret]];
...
- (void)showSharView{
    ...
    [[HttpClient defaultClient] requestWithPath:mGetConf method:1 parameters:paraDict prepareExecute:^{
        ...
        if ([rescode isEqualToString:@"1"]) {
            ...
            LIMLiveShareView *shareView = [[LIMLiveShareView alloc]init];
            shareView.ewmshareurl = shareUrl;
            shareView.coverUrl = self.coverUrl;
            [shareView setUI];
        }else{
            NSString * resmsg = responseObject[@"resmsg"];
            NSLog(@"resmsg = %@",resmsg);
...
//    NSLog(@"送出了第%ld的礼物，%@个 ",self.currentGiftIndex,self.currentGiftCount);
    ...
    [[HttpClient defaultClient] requestWithPath:mGive_gift method:1 parameters:paraDict prepareExecute:^{
        ...
        if ([rescode isEqualToString:@"1"]) {
            ...
            // gift
            LiveUserModel *fourthUser = [[LiveUserModel alloc]init];
            fourthUser.name = userModel.nickname;
            fourthUser.iconUrl = userModel.avatar;
...
//            [self bigGiftAnimation:giftModel.giftid];
...
- (void)bigGiftAnimation:(LIMGiftArrModel *)giftArrModel{
    ...
    if(!self.isAnimation){
        ...
        if ([giftID isEqualToString:@"4"]||[giftID isEqualToString:@"6"]||[giftID isEqualToString:@"14"]||[giftID isEqualToString:@"17"]||[giftID isEqualToString:@"18"]||[giftID isEqualToString:@"19"]||[giftID isEqualToString:@"25"]||[giftID isEqualToString:@"28"]||[giftID isEqualToString:@"29"]||[giftID isEqualToString:@"31"]) {
            ...
            //            UIImageView *imageV_smoke = [[UIImageView alloc]init]; // 烟雾
            //
            //            NSMutableArray *imageArray_smoke = [[NSMutableArray alloc] init];
            //            for (int i = 0; i < 15; i++) {
            //                NSString *imageName = [NSString stringWithFormat:@"%@%02d", @"cake_smoke_000",i];
            //                NSString *path = [[NSBundle mainBundle] pathForResource:imageName ofType:@"png"];
            //                UIImage *image = [UIImage imageWithContentsOfFile:path];
            //
            //                //            UIImage *image = [UIImage imageNamed:imageName];
            //                [imageArray_smoke addObject:image];
            //            }
            //            imageV_smoke.animationImages = imageArray_smoke;// 序列帧动画的uiimage数组
            //            imageV_smoke.animationDuration = 1.2f;// 序列帧全部播放完所用时间
            //            imageV_smoke.animationRepeatCount = 1;// 序列帧动画重复次数
            ...
            //            UIImageView *imageV_happy = [[UIImageView alloc]initWithFrame:CGRectMake(0, 240*kiphone6, kScreenW, kScreenH)]; // happy
            //
            //            //        [imageV_happy mas_makeConstraints:^(MASConstraintMaker *make) {
            //            //            make.top.equalTo(self.bigGift).offset(480 *kiphone6).priority(750);
            //            //            make.centerX.equalTo(self.bigGift).offset(0);
            //            //            make.width.mas_equalTo(kScreenW);
            //            //            make.height.mas_equalTo(kScreenH);
            //            //        }];
            //            NSMutableArray *imageArray_happy = [[NSMutableArray alloc] init];
            //            for (int i = 0; i < 20; i++) {
            //                NSString *imageName = [NSString stringWithFormat:@"%@%02d", @"cake_happy_000",i];
            //                NSString *path = [[NSBundle mainBundle] pathForResource:imageName ofType:@"png"];
            //                UIImage *image = [UIImage imageWithContentsOfFile:path];
            //
            //                //            UIImage *image = [UIImage imageNamed:imageName];
            //                [imageArray_happy addObject:image];
            //            }
            //            imageV_happy.animationImages = imageArray_happy;// 序列帧动画的uiimage数组
            //            imageV_happy.animationDuration = 4.8f;// 序列帧全部播放完所用时间
            //            imageV_happy.animationRepeatCount = 1;// 序列帧动画重复次数
            
            
            
            //            UIImageView *imageV_qiqiu = [[UIImageView alloc]init]; // 气球
            //
            //            NSMutableArray *imageArray_qiqiu = [[NSMutableArray alloc] init];
            //            for (int i = 0; i < 16; i++) {
            //                NSString *imageName = [NSString stringWithFormat:@"%@%02d", @"cake_balloon_000",i];
            //                NSString *path = [[NSBundle mainBundle] pathForResource:imageName ofType:@"png"];
            //                UIImage *image = [UIImage imageWithContentsOfFile:path];
            //
            //                //            UIImage *image = [UIImage imageNamed:imageName];
            ...
            //            }
            //            imageV_qiqiu.animationImages = imageArray_qiqiu;// 序列帧动画的uiimage数组
            //            imageV_qiqiu.animationDuration = 1.6f;// 序列帧全部播放完所用时间
            //            imageV_qiqiu.animationRepeatCount = 1;// 序列帧动画重复次数
            
            
            
            
            //            UIImageView *imageV_star = [[UIImageView alloc]init]; // 星星
            //            NSMutableArray *imageArray_star = [[NSMutableArray alloc] init];
            //            for (int i = 0; i < 46; i++) {
            //                NSString *imageName = [NSString stringWithFormat:@"%@%02d", @"cake_star_000",i];
            //                NSString *path = [[NSBundle mainBundle] pathForResource:imageName ofType:@"png"];
            //                UIImage *image = [UIImage imageWithContentsOfFile:path];
            //
            //                //            UIImage *image = [UIImage imageNamed:imageName];
            //                [imageArray_star addObject:image];
            //            }
            //            imageV_star.animationImages = imageArray_star;// 序列帧动画的uiimage数组
            //            imageV_star.animationDuration = 3.5f;// 序列帧全部播放完所用时间
            //            imageV_star.animationRepeatCount = 1;// 序列帧动画重复次数
            ...
            //            [self.bigGift addSubview:imageV_star];
            [self.bigGift addSubview:imageV_cake];
            //            [self.bigGift addSubview:imageV_smoke];
            [self.bigGift addSubview:imageV_birthday];
            //            [self.bigGift addSubview:imageV_happy];
            //            [self.bigGift addSubview:imageV_qiqiu];
            //            [self.bigGift bringSubviewToFront:imageV_star_new];
            //            [self.bigGift bringSubviewToFront:imageV_cake];
            //            [self.bigGift bringSubviewToFront:imageV_smoke_new];
            //            [self.bigGift bringSubviewToFront:imageV_birthday];
            //            [self.bigGift bringSubviewToFront:imageV_happy_new];
            //            [self.bigGift bringSubviewToFront:imageV_qiqiu_new];
            ...
            //            [imageV_qiqiu_new mas_makeConstraints:^(MASConstraintMaker *make) {
            //                make.top.equalTo(self.bigGift).offset(0);
            //                make.centerX.equalTo(self.bigGift).offset(0);
            //                make.width.mas_equalTo(kScreenW);
            //                make.height.mas_equalTo(kScreenH);
            //            }];
            
            //            [imageV_star_new mas_makeConstraints:^(MASConstraintMaker *make) {
            //                make.top.equalTo(self.bigGift).offset(0);
            //                make.centerX.equalTo(self.bigGift).offset(0);
            //                make.width.mas_equalTo(kScreenW);
            //                make.height.mas_equalTo(kScreenH);
...
- (void)giftChat:(NSString *)giftCount andGift:(LIMPresentModel *)gift{
    CcUserModel *userModel = [CcUserModel defaultClient];
    NSLog(@"%@,%@,%@,",userModel.avatar,userModel.nickname,userModel.uid);
    NSDictionary *dict = @{
                           @"headPic":userModel.avatar,
...
//                           @"level":self.livePushModel.level,
    ...
    [grp_conversation sendMessage:msg succ:^(){
        NSLog(@"SendMsg Succ");
        NSString *string = [NSString stringWithFormat:@"%@ 送给了主播%ld%@%@",userModel.nickname,[giftCount integerValue],gift.unit,gift.giftname];
        LIMChatModel *chatModel = [[LIMChatModel alloc]init];
        chatModel.nameLength = userModel.nickname.length +1;
        chatModel.sentence = string;
        chatModel.type = @"6";
...
//    [self loginAnimation:userModel.userlevel];

    [self loginAnimation:userModel.userlevel andUserName:userModel.nickname andUserIcon:userModel.avatar];
    NSDictionary *dict = @{
                           @"headPic":userModel.avatar,
                           @"msg":text,
                           @"nickName":userModel.nickname,
    ...
    [grp_conversation sendMessage:msg succ:^(){
        NSLog(@"SendMsg Succ");
        NSString *string = [NSString stringWithFormat:@"%@ %@",userModel.nickname,text];
        LIMChatModel *chatModel = [[LIMChatModel alloc]init];
        chatModel.nameLength = userModel.nickname.length +1;
        chatModel.sentence = string;
        chatModel.type = @"2";
...
- (void)shareChat{
    ...
    NSDictionary *dict = @{@"headPic":userModel.avatar,
                           @"msg":text,
                           @"nickName":userModel.nickname,
                           @"userAction":@(7),
                           @"userId":userModel.uid,
...
- (void)followChat{
    ...
    NSDictionary *dict = @{@"headPic":userModel.avatar,
                           @"msg":text,@"nickName":userModel.nickname,
                           @"userAction":@(8),
                           @"userId":userModel.uid,
                           @"level":userModel.userlevel,
                           @"isHost":userModel.iscompere
...
- (void)loginAnimation:(NSString *)userLevel andUserName:(NSString *)userName andUserIcon:(NSString *)iconUrl{
    ...
    if (self.loginArr.count >0) {
        ...
        if (self.loginArr.count > 0) {
//            NSString *string = self.loginArr[0];
