...
@interface LIMPushAdminViewController ()
<
...
@implementation LIMPushAdminViewController
...
- (void)viewDidLoad {
    ...
    //    int i =arc4random() %2;
    //
    //    NSDictionary *dic =@{@"msg":@"123;lksal;",@"msgID":@"1",@"sender":[NSString stringWithFormat:@"%zd",i],@"sendTime":@"06-23",@"msgType":@"0"};
    //    SDChatMessage *msg =[SDChatMessage chatMessageWithDic:dic];
    //    SDChatDetail *chat =[SDChatDetail sd_chatWith:msg];
    //    SDChatDetailFrame *chatFrame =[[SDChatDetailFrame alloc]init];
    //    chatFrame.chat=chat;
...
-(SDChatDetailTableView *)chatTableView{
    if(!_chatTableView){
        _chatTableView = [[SDChatDetailTableView alloc] initWithFrame:CGRectMake(0,kScreenH -kPushMsgH +44, SDDeviceWidth, kPushMsgH -44) style:UITableViewStylePlain];
...
- (void)onNewMessage:(NSArray*) messages {
    ...
    if ([msg.sender isEqualToString:_chatListModel.kkID]) {
        ...
        message_sd.sendIconUrl = @"msglist_2";
