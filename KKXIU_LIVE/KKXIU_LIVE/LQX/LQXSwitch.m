...
- (void)layoutSubviews
{
    ...
    CGFloat r = 5.0f;//CGRectGetHeight(self.containerView.bounds) / 2.0;
    ...
    if (!self.isOn) {
        ...
        self.knobView.frame = CGRectMake(1,
                                         margin,
                                         self.ballSize *2,
                                         self.ballSize);
    } else {
        ...
        self.knobView.frame = CGRectMake(CGRectGetWidth(self.containerView.bounds) - 1 - self.ballSize*2,
                                         margin,
                                         self.ballSize *2,
                                         self.ballSize);
    }
...
- (void)setOn:(BOOL)on animated:(BOOL)animated
{
    ...
    _on = on;
    CGFloat marginX = (CGRectGetHeight(self.bounds) - self.ballSize *2) / 2.0;
    CGFloat margin = (CGRectGetHeight(self.bounds) - self.ballSize ) / 2.0;
    ...
    if (!animated) {
        if (!self.isOn) {
            ...
            self.midLabel.textColor = self.offColor;
            ...
            self.midLabel.textColor = self.onColor;
        }
    } else {
        if (self.isOn) {
...
- (void)commonInit
{
    ...
    _knobView = [[UIView alloc] initWithFrame:CGRectMake(1, 0, self.ballSize *2, self.ballSize)];
    _knobView.backgroundColor = _thumbTintColor;
    _knobView.layer.cornerRadius = 4;
    [_containerView addSubview:_knobView];
    
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake((self.ballSize *2-50-3)/2.0, (self.ballSize -25)/2.0, 55, 25)];
    titleLabel.backgroundColor = [UIColor clearColor];
    titleLabel.textAlignment = NSTextAlignmentCenter;
    titleLabel.textColor = self.offColor;
    titleLabel.font = _font;
    titleLabel.text = @"弹幕";
    
    [_knobView addSubview:titleLabel];
    
    self.midLabel = titleLabel;
...
- (void)handlePanGestureRecognizerEvent:(UIPanGestureRecognizer *)recognizer
{
    CGFloat margin = (CGRectGetHeight(self.bounds) - self.ballSize *2) / 2.0;
    CGFloat offset = 6.0f;
    ...
    switch (recognizer.state) {
        ...
        case UIGestureRecognizerStateFailed: {
            if (!self.isOn) {
                ...
                [UIView animateWithDuration:0.25
                                 animations:^{
                                     self.knobView.frame = CGRectMake(CGRectGetWidth(self.containerView.bounds) - self.ballSize*2-1,
                                                                      margin,
                                                                      self.ballSize*2,
                                                                      self.ballSize);
                                 }];
