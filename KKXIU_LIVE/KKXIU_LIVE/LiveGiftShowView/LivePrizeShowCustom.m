...
@property (nonatomic, strong) NSMutableArray <LivePrizeShowModel *> * waitQueueArr;// 待展示礼物队列

@end

@implementation LivePrizeShowCustom

#pragma mark - 初始化
+ (instancetype)addToView:(UIView *)superView{
    LivePrizeShowCustom * v = [[LivePrizeShowCustom alloc]init];
    v.kExchangeAnimationTime = 0.25;
    v.kAppearAnimationTime = 0.8;
...
- (void)addLiveGiftShowModel:(LivePrizeShowModel *)showModel{
    [self addLiveGiftShowModel:showModel showNumber:0];
}

- (void)addLiveGiftShowModel:(LivePrizeShowModel *)showModel showNumber:(NSInteger)showNumber{
    if (!showModel || ![showModel isKindOfClass:[LivePrizeShowModel class]]) {
        return;
    }
    
    LivePrizeShowView * oldShowView = [self.showViewDict objectForKey:[self getDictKey:showModel]];
    
    //判断是否强制修改显示的数字
    BOOL isResetNumber = showNumber > 0 ? YES : NO;

    //如果不存在旧模型
    if (!oldShowView || ![oldShowView isKindOfClass:[LivePrizeShowView class]]) {
        //如果当前弹幕数量大于最大限制
        if (self.showViewArr.count >= live_maxGiftShowCount) {
            ...
            if (![self.showViewArr containsObject:kGiftViewRemoved]) {
                if (self.addMode == LiveGiftAddModeReplace) {
                    //排序 最小的时间在第一个
                    NSArray * sortArr = [self.showViewArr sortedArrayUsingComparator:^NSComparisonResult(LivePrizeShowView * obj1, LivePrizeShowView * obj2) {
                        return [obj1.creatDate compare:obj2.creatDate];
                    }];
                    LivePrizeShowView * oldestView = sortArr.firstObject;
                    //重置模型
...
//            frame = CGRectMake(-[UIScreen mainScreen].bounds.size.width, showViewY, 0, 0);
            frame = CGRectMake([UIScreen mainScreen].bounds.size.width, showViewY, 0, 0);
//        }
        LivePrizeShowView * newShowView = [[LivePrizeShowView alloc]initWithFrame:frame andType:showModel.type];
        //赋值
        ...
        newShowView.liveGiftShowViewTimeOut = ^(LivePrizeShowView * willReMoveShowView){
...
- (void)animatedWithGiftModel:(LivePrizeShowModel *)showModel {
    if (!showModel) { return ;}
    if (self.addMode == LiveGiftAddModeAdd) {
        ...
        if (!oldShowView) {// 不存在旧视图
            ...
            for (id object in self.showViewArr) {
                if ([object isKindOfClass:[LivePrizeShowView class]]) {
                    showCount ++;
                }
            }
...
- (void)appearWith:(LivePrizeShowView *)showView {
    // 出现的动画
//    if (live_appearModel == LiveGiftAppearModeLeft) {
        ...
        [UIView animateWithDuration:0.5 animations:^{
            ...
            NSLog(@"%g",f.origin.x);
            f.origin.x = [UIScreen mainScreen].bounds.size.width -230;
            NSLog(@"%g",f.origin.x);
...
- (void)resetY {
    for (int i = 0; i < self.showViewArr.count; i++) {
        LivePrizeShowView * show = self.showViewArr[i];
        if ([show isKindOfClass:[LivePrizeShowView class]]) {
...
- (void)sortShowArr{
    ...
    for (int i = 0; i < self.showViewArr.count; i++) {
        for (int j = i; j < self.showViewArr.count; j++) {
            LivePrizeShowView * showViewI = self.showViewArr[i];
            LivePrizeShowView * showViewJ = self.showViewArr[j];
            if ([showViewI isKindOfClass:[LivePrizeShowView class]] && [showViewJ isKindOfClass:[LivePrizeShowView class]]) {
...
- (void)searchLiveShowViewFrom:(int)i{
    for (int j = i; j < self.showViewArr.count; j++) {
        LivePrizeShowView * next = self.showViewArr[j];
        if ([next isKindOfClass:[LivePrizeShowView class]]) {
...
#pragma mark - Private
- (void)addToQueue:(LivePrizeShowModel *)showModel {
    if (!showModel) {
        return;
    }
    ...
    for (NSUInteger i = 0; i<self.waitQueueArr.count; i++) {
        LivePrizeShowModel * oldModel = self.waitQueueArr[i];
...
- (void)showWaitView {
    ...
    if (showCount < live_maxGiftShowCount) {
        LivePrizeShowModel * model = self.waitQueueArr.firstObject;
        [self animatedWithGiftModel:model];
        [self.waitQueueArr removeObject:model];
    }
}

- (void)resetView:(LivePrizeShowView *)view nowModel:(LivePrizeShowModel *)model isChangeNum:(BOOL)isChange number:(NSInteger)number{
...
- (NSString *)getDictKey:(LivePrizeShowModel *)model{
    //默认以 用户名+礼物类型 为key
...
#pragma mark - Lazy
- (NSMutableArray<LivePrizeShowModel *> *)waitQueueArr {
    if (!_waitQueueArr) {
