...
static CGFloat const kNameLabelFont = 11.0;//送礼者
...
@property (nonatomic ,strong) UIImageView * bigPrizeIV;/**< 手气逆天了 */
...
@implementation LivePrizeShowView
...
- (void)setModel:(LivePrizeShowModel *)model{
    if (!model) {
        return;
    }
    _model = model;
    
    self.nameLabel.attributedText = model.user.name;
    
    [self.iconIV sd_setImageWithURL:[NSURL URLWithString:model.user.iconUrl] placeholderImage:[UIImage imageNamed:@"place_icon"]];
    ...
    self.bigPrizeIV.image = [UIImage imageNamed:@"组-1"];
...
- (void)setupContentContraints{
    ...
    [self.iconIV mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(self.mas_left).offset(5);
...
//        make.left.equalTo(self.nameLabel.mas_right).offset(5).priority(750);
    ...
    [self.bigPrizeIV sizeToFit];
    [self.bigPrizeIV mas_makeConstraints:^(MASConstraintMaker *make) {
        //        make.left.equalTo(self.nameLabel.mas_right).offset(5).priority(750);
...
//        make.left.equalTo(self.mas_right).offset(50).priority(750);
...
- (UIImageView *)iconIV{
    if (!_iconIV) {
        ...
        _iconIV.layer.cornerRadius = 20.5;
...
- (UIImageView *)bigPrizeIV{
    if (!_bigPrizeIV) {
        _bigPrizeIV = [self creatIV];
    }
    return _bigPrizeIV;
}

- (LivePrizeShowNumberView *)numberView{
    if (!_numberView) {
//        LivePrizeShowNumberView * nv = [[LivePrizeShowNumberView alloc]init];
//        [self addSubview:nv];
//        _numberView = nv;
    }
    return _numberView;
