//
//  LIMPushChatDetaiViewController.m
//  KKXIU_LIVE
//
...
@interface LIMPushChatDetaiViewController ()
<
...
@implementation LIMPushChatDetaiViewController
...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillShow:) name:@"keybordshow" object:nil];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self setTitle:@"会话"];
//    self.navView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kScreenW, kScreenH)];
//    self.navView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.5];
//    [self.view addSubview:_navView];
    [self setUI];
    
    NSLog(@"%@",_chatListModel.kkID);
...
-(SDChatDetailTableView *)chatTableView{
    if(!_chatTableView){
        _chatTableView = [[SDChatDetailTableView alloc] initWithFrame:CGRectMake(0,kScreenH -300, SDDeviceWidth, 300- 50) style:UITableViewStylePlain];
...
-(void)setUI{
    ...
    [settingBtn addTarget:self action:@selector(dismissThisView) forControlEvents:UIControlEventTouchUpInside];
    ...
    [self.bjView addSubview:hiddenView];
    UITapGestureRecognizer *hiddenGest = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(resignInput)];
...
//                                                          [self.chatTableView setNeedsLayout];
                                                          CGRect frame =   self.bjView.frame;
                                                          frame.origin.y -=100;
...
//    CGFloat chatInputViewHeight =CGRectGetHeight(self.chatInputView.frame);
//    后来 SDDeviceHeight-keyboardHeight-(chatInputViewHeight-keyBoardDefaultHeight);
    CGFloat systemKeyBoardHeight =[notification.userInfo[@"UIKeyboardBoundsUserInfoKey"]CGRectValue].size.height;
    NSLog(@"keyboard = %g",systemKeyBoardHeight);
//    NSLog(@"keyboard = %@",(NSString *)notification.object);
    
    [UIView animateWithDuration:0.5 animations:^{
        CGRect frame = self.bjView.frame;
//        frame.origin.y = [(NSString *)notification.object floatValue]-44 -19;
        frame.origin.y = -systemKeyBoardHeight;
        self.bjView.frame = frame;
